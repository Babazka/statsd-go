  ...
  */
  ...
  if (bitscmp(PARSE_DASHES)) {
    ...
    if (dashes) {
      ...
      /* count the , in  dashes */
      char *t;
      int cnt=0;for(t=dashes;(*t)&&(t=strchr(t,','));t++,cnt++) {;}
...
int parse_line(enum gf_en gf,parsedargs_t*pa,image_desc_t *const im){
  ...
  if (gdp->ndash) {
    ...
    for(i=1;i<gdp->ndash;i++){dprintf(", %g",gdp->p_dashes[i]);}
    dprintf("\n");
  }
  dprintf("=================================\n");
